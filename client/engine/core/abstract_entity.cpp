...
namespace engine {
  ...
  void AbstractEntity::setVisible(bool visible) {
    if (m_visible != visible) {
      m_visible = visible;
      qDebug() << this->objectName() << "is" << (visible ? "visible" : "invisible");
  ...
  void AbstractEntity::setSelected(bool selected) {
    ...
    qDebug() << this->objectName() << "is selected";
  ...
  void AbstractEntity::setWireFrameMode(bool enabled) {
    if (m_wireFrameMode != enabled) {
      ...
      qDebug() << "Wireframe mode of " << this->objectName() << "is" << (enabled ? "enabled" : "disabled");
  ...
  void AbstractEntity::setPosition(QVector3D position) {
    if (isnan(position)) {
      qDebug() << "Failed to set position: NaN detected";
      return;
    }

    if (!isEqual(m_position, position)) {
      m_position = position;
      qDebug() << "The position of" << this->objectName() << "is set to" << position;
  ...
  void AbstractEntity::setRotation(QVector3D rotation) {
    if (isnan(rotation)) {
      qDebug() << "Failed to set rotation: NaN detected";
      return;
    }

    if (!isEqual(m_rotation, rotation)) {
      m_rotation = rotation;
      qDebug() << "The rotation of" << this->objectName() << "is set to" << rotation;
  ...
  void AbstractEntity::setScaling(QVector3D scaling) {
    if (isnan(scaling)) {
      qDebug() << "Failed to set scaling: NaN detected";
      return;
    }

    if (!isEqual(m_scaling, scaling)) {
      m_scaling = scaling;
      qDebug() << "The scaling of" << this->objectName() << "is set to" << scaling;
